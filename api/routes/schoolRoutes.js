...
router.get("/", authMiddleware, async (req, res) => {
    try {
        ...
        if (user.role === "admin") {
            // Admin: Tüm okulları tüm verileriyle görebilir
            schools = await prisma.school.findMany({
                include: { students: true, teachers: true, manager: true },
            });
        } else if (user.role === "manager") {
            // Manager: Tüm okulları görebilir ama sadece genel bilgiler (detaylar için kendi okuluna erişim)
            ...
            // Her okul için öğrenci ve öğretmen sayısını al
            schools = await Promise.all(
                allSchools.map(async (school) => {
                    ...
                    return {
                        ...
                        isMySchool: school.managerId === user.id,
                    };
                })
            );
